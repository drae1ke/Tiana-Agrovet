...
import { getProducts, updateProduct, addSale } from '@/utils/storage';
...
const POS: React.FC = () => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter(p => 
      p.quantity > 0 && (
        ...
        p.nameSwahili.toLowerCase().includes(query) ||
  ...
  const processPayment = async () => {
    ...
    const sale: Sale = {
      ...
      total: cartTotal,
      paymentMethod,
      mpesaRef: paymentMethod === 'mpesa' ? `MP${Date.now().toString(36).toUpperCase()}` : undefined,
                  ...
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <Badge variant="outline" className="text-xs">
                        {product.quantity} {language === 'sw' ? 'zilizo' : 'in stock'}
                      </Badge>
                    </div>
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={processPayment} disabled={isProcessing}>
              {isProcessing ? t('loading') : t('processPayment')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
